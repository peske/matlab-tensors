...
% Check if the input argument is a tensor of the specified order.
%
...
% specify ``order = 3``, the function will return ``true`` if ``tsr`` is a
% scalar, vector, matrix or a three-dimensional tensor.
%
% Args:
%   tsr: Value to check.
%   order: The order of the tensor. It has to be a scalar, positive whole 
%       number that represents number of dimensions. For example, a scalar
%       is a zero-order tensor, a vector is a first-order tensor, a matrix
%       is a second-order tensor, a three-dimensional array is a
%       third-order tensor, etc. If this argument isn't specified, or if
%       it's empty, it will default to ``3``.
%
% Returns:
...
%       if ``tsr`` is a vector of size ``[4, 1]``, and ``order`` is ``4``,
%       the resulting ``tSize`` will be ``[4, 1, 1, 1]``. But there are
%       also two cases when it may happen that ``tSize`` has length greater 
%       than ``order``:
%
%       - if ``tsr`` has more dimensions than specified by ``order``, in
%         which case ``isTsr`` will be false;
%       - If ``order`` is less than ``2``. Minimal length of ``tSize`` is
%         ``2``, as in
%         `size <https://www.mathworks.com/help/matlab/ref/size.html>`_
%         function.
%
