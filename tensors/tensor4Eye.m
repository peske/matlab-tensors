...
%   Input arguments:
%
%    m -       1st and 3rd dimension of the resulting tensor4. It has to be a scalar with positive integer value.
%
...
%    te -      The resulting eye tensor4 (identity tensor4).
    ...
    if ~isscalar(m) || ~isWholeNumber(m) || m < 1
        error('The first argument (''m'') has to be a scalar with a positive integer value.');
    end

    if nargin < 2 || isempty(n)
        n = m;
    elseif ~isscalar(n) || ~isWholeNumber(n) || n < 1
        error('The second argument (''n'') has to be a scalar with a positive integer value.');
    end
