...
% Create a tensor of specified size from the input values.
%
% Args:
%   tSize: Desired size of the tensor. It is a vector whose elements are
%       representing desired lengths per dimension. Every non-zero element
%       of this vector represents the length of the corresponding dimesion
%       of the resulting tensor. For zero elements the length of the
%       corresponding dimension of the resulting tensor will be inferred
%       from other arguments, if possible. If it cannot be inferred, an
%       error will be thrown. If this argument is a scalar, then it
%       represents the desired order (total number of dimensions) of the
%       resulting tensor, and the specified lengths per dimension are
%       assumed to be ``0``. For example, setting this argument to ``4``
%       has the same meaning as setting it to four-element zero row vector
%       ``[0, 0, 0, 0]``.
%   a: The value which will be used for creating the tensor. It can be a
%       scalar, vector, matrix, or a tensor. If it's a vector, it isn't
%       important if it is a row or column vector - the result will be the
%       same.
...
%       along 3rd dimension of the resulting tensor. Number of elements of
%       ``dimA`` must be equal to number of dimensions of value ``a``. If
%       ``dimA`` isn't provided, or is empty, it's assumed that ``a``
%       should end up along the first dimensions of the resulting tensor,
%       so it'll get the following values by default:
%
...
%         that matrix ``a`` should end up along the first two dimensions of
%         the resulting tensor;
%       - if ``a`` is a three-dimensional tensor, ``dimA`` defaults to
%         ``[1, 2, 3]``, etc.
%   varargin: The function accepts variable number of arguments so that we
%       can provide multiple ``a`` / ``dimA`` pairs, every representing an
%       input tensor. These tensors are then multiplied element-wise to
